...
\begin{itemize}
	...
	\end{align*}
	\item So $f'(3) \simeq 6$, so their difference is infinitesimal. But they're both \textit{real numbers}, so their difference has to be real. The only real infinitesimal is $0$, so their difference is $0$. Thus $f'(3) = 6$.
\end{itemize}
\end{frame}
...
Call $f(x)$ \textit{continuous} at a point $c$ if for every hyperreal $x \simeq c$, we have $f(x) \simeq c$.
\begin{theorem}
	If $f(x)$ is differentiable at $c$, then it is continuous at $c$.
\end{theorem}
For any $x \simeq c$, we have $x - c$ infinitesimal. Then $f'(c) = \stp{\frac{f(x) - f(c)}{x - c}}$. If $f(x) - f(c)$ wasn't infinitesimal, then since $x - c$ is infinitesimal, $\frac{f(x) - f(c)}{x-c}$ would be ``infinite'' and there would be no standard part. Hence $f(x) - f(c)$ is infinitesimal, hence $f(x) \simeq f(c)$.
\end{frame}